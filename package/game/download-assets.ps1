# Run ONCE on a Windows machine with internet, from the game folder (PowerShell).
$base="https://d8j0ntlcm91z4.cloudfront.net/user_36vHR3g6PFbZ743GYNcYpzsyXRd"
New-Item -ItemType Directory -Force -Path "assets/cdn" | Out-Null
@"
...
"@ -split "`n" | ForEach-Object {
  $f = $_.Trim(); if ($f -eq "") { return }
  Write-Host "  $f"
  Invoke-WebRequest -Uri "$base/$f" -OutFile "assets/cdn/$f"
}
Write-Host "All assets mirrored. The game is now fully offline."
